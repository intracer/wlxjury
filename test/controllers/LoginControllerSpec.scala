...
class LoginControllerSpec extends PlaySpecification with Results with TestDb with Mockito {
  ...
  val login = new LoginController(usersController)
...
//    }
    ...
    "root rights" in {
      withDb {
        ...
        val setCookies = cookies(result)
        val setHeaders = headers(result)
        println("cookies: " + setCookies)
        println("headers: " + setHeaders)
        println("")
        ok
