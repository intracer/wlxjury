...
function UGTileDesign(){
	...
	function setHtmlThumb(objThumbWrapper, objItem){
		...
		if(g_options.tile_enable_icons){
			...
			var toSeparateIcon = g_temp.isSaparateIcons;
			if(toSeparateIcon == false && objItem.type != "image" && g_temp.isVideoplayIconAlwaysOn == true)
				toSeparateIcon = true;

			if(toSeparateIcon)		//put the icons on the thumb
			...
			var objButtonSelect = objOverlay.children(".ug-icon-select");

			if(objButtonSelect.length == 0)
				objButtonSelect = null;
			else
				objButtonSelect.hide();

			//if only zoom icon, make the tile clickable for lightbox open
			if(!objButtonSelect && g_options.tile_enable_action == true)
	...
	function getButtonSelect(objTile){
		var objButton = objTile.find(".ug-icon-select");
		if(objButton.length == 0)
			return(null);
	...
	function getButtonReject(objTile){
		var objButton = objTile.find(".ug-icon-reject");
		if(objButton.length == 0)
			return(null);
	...
	function positionElements(objTile){
		...
		var objButtonSelect = getButtonSelect(objTile);
		var objButtonReject = getButtonReject(objTile);
		...
		if(objButtonZoom || objButtonSelect){
		...
		if(objButtonZoom && objButtonSelect){
			...
			var sizeSelect = g_functions.getElementSize(objButtonSelect);
			...
			if(objButtonSelect)
				g_functions.placeElement(objButtonSelect, "center", "middle", 0, gapVert);
		...
		if(objButtonSelect)
			objButtonSelect.show();

		if(objButtonReject)
			objButtonReject.show();
	...
	function onSelectButtonClick(event){
		...
		g_objThis.trigger(t.events.SELECT_CLICK, objTile);
	}

	/**
	 * on reject icon click
	 */
	function onRejectButtonClick(event){
		var objTile = jQuery(this).parents(".ug-tile");
		...
		g_objThis.trigger(t.events.REJECT_CLICK, objTile);
	}
	...
	this.initEvents = function(){
		...
		g_objParentWrapper.delegate(".ug-tile .ug-icon-select", "click", onSelectButtonClick);
		g_objParentWrapper.delegate(".ug-tile .ug-icon-reject", "click", onRejectButtonClick);
	}
