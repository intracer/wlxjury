...
public class Wiki implements Serializable {
    ...
    public byte[] getImage(String title, int width, int height) throws IOException {
        ...
        String url2 = getImageUrl(title, width, height);
    ...
    public String getImageUrl(String title, int width, int height) throws IOException {
        // this is a two step process - first we fetch the image url
        String titleWONamespace = title.replaceFirst("^(File|Image|" + namespaceIdentifier(FILE_NAMESPACE) + "):", "");
        StringBuilder url = new StringBuilder(query);
        url.append("prop=imageinfo&iiprop=url&titles=File:");
        url.append(URLEncoder.encode(normalize(titleWONamespace), "UTF-8"));
        url.append("&iiurlwidth=");
        url.append(width);
        url.append("&iiurlheight=");
        url.append(height);
        String line = fetch(url.toString(), "getImage");
        return parseAttribute(line, "url", 0);
    }
    ...
    protected LogEntry[] getIPBlockList(String user, Calendar start, Calendar end) throws IOException {
        ...
        // url base
        StringBuilder url***REMOVED*** = new StringBuilder(query);
        url***REMOVED***.append("list=blocks&bklimit=");
        url***REMOVED***.append(max);
        if (end != null) {
            url***REMOVED***.append("&bkend=");
            url***REMOVED***.append(calendarToTimestamp(end));
        }
        if (!user.isEmpty()) {
            url***REMOVED***.append("&bkusers=");
            url***REMOVED***.append(user);
        }
        url***REMOVED***.append("&bkstart=");

        // connection
        ArrayList<LogEntry> entries = new ArrayList<LogEntry>(1333);
        do {
            String line = fetch(url***REMOVED***.toString() + bkstart, "getIPBlockList");
